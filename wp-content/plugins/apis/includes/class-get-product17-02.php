...
function GetRecipieDetails($product_lists){
	...
	$recipie_id = implode(',', $recipie_id);
	$recipie = $wpdb->get_results("select * from wp_recipe where product_list = '$recipie_id'",'ARRAY_A') ;
	for( $m=0; $m<count($product_lists); $m++){
		$product_lists[$m]['recipie_details'] = $recipie[0];
		$prod_id = $product_lists[$m]['recipie_details']['product_list'];
		$product_lists[$m]['recipie_details']['product_list'] = $wpdb->get_results("select * from `wp_product` where id IN ($prod_id)","ARRAY_A");
		}
	}else{
		for( $i=0; $i<count($product_lists); $i++){
		$recipie = $product_lists[$i]['id'];
		$product_lists[$i]['recipie_details'] = $wpdb->get_results("select * from wp_recipe where product_list LIKE '%$recipie%'",'ARRAY_A');
